...
const firstNames = ['Олександр', 'Максим', 'Дмитро', 'Данило', 'Михайло', 'Іван', 'Артем', 'Андрій', 'Владислав', 'Богдан'];
const lastNames = ['Шевченко', 'Бондаренко', 'Ковальчук', 'Мельник', 'Коваленко', 'Бойко', 'Ткаченко', 'Кравченко', 'Олійник', 'Шевчук'];

// Украинские города
const cities = ['Київ', 'Львів', 'Харків', 'Одеса', 'Дніпро', 'Запоріжжя', 'Вінниця', 'Полтава', 'Черкаси', 'Житомир'];

// Типы карт
const cardTypes = ['Classic', 'Gold', 'Platinum', 'World'];

// Типы транзакций
const transactionTypes = ['Продукти', 'Комунальні послуги', 'Розваги', 'Транспорт', 'Одяг', 'Здоров\'я', 'Подарунки', 'Техніка'];
...
const generateUser = async (index) => {
  ...
  const user = {
    ...
    profile: {
      ...
      dateOfBirth: randomDate(new Date(1980, 0, 1), new Date(2000, 11, 31)).toISOString().split('T')[0],
      presentAddress: `вул. ${lastName}а, ${randomInt(1, 100)}`,
      permanentAddress: `вул. ${lastName}а, ${randomInt(1, 100)}`,
      city: city,
...
const generateLoan = (userId, index) => {
  ...
  return {
    ...
    nextPaymentDate: new Date(startDate.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString(),
    createdAt: new Date().toISOString()
  };
};
